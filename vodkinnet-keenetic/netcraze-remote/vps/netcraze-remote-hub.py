    ...
    return json.dumps(
        {
            ...
            "theme_color": "#ff6a00",
            "description": "Удалённый доступ к Keenetic/KNDMS через свой VPS",
            "icons": [
                {
                    "src": "/favicon.svg",
                    "sizes": "any",
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
  ...
  <rect x="16" y="25" width="32" height="18" rx="6" fill="none" stroke="#38bdf8" stroke-width="3"/>
  <circle cx="26" cy="34" r="3" fill="#22c55e"/>
  <circle cx="38" cy="34" r="3" fill="#22c55e"/>
  <rect x="22" y="47" width="20" height="4" rx="2" fill="#2f8fff"/>
</svg>
""".strip() + "\n"
...
def init_db(conn):
    ...
    conn.execute(
        ...
        create table if not exists routers (
            ...
            vless_port integer not null default 8444,
...
def make_router_conf(row, hub_url):
    """VodkinNET: KNDMS/Entware не имеет uci — это единственная функция во
    всём файле, которая реально завязана на механизм хранения конфига на
    роутере. Формат вывода — обычный shell key="value" файл (агент читает
    его через sed, не source'ит) вместо uci-команд. Схема полей (admin_*/
    ssh_*, vless_uuid, ssh_vless_uuid) — один в один с owrt-remote, чтобы
    весь остальной код Hub'а (5000+ строк дашборда/JS/терминала/метрик) не
    пришлось трогать вообще.

    ВАЖНО: reverse-tag НЕ берётся из колонки БД reverse_tag/ssh_reverse_tag
    (там у всех роутеров одинаковый дефолт 'reverse-in' — легаси-поле).
    Реальный tag, который server-side make_server_xray_config() реально
    генерирует и матчит, детерминирован от router_id: reverse-{id} и
    reverse-{id}-ssh. Раньше здесь ошибочно шла колонка БД — с одним
    роутером могло случайно "сработать", а со вторым тэги гарантированно
    разъехались бы и туннель не поднялся бы."""
    router_id = clean_router_id(row["id"])
    reverse_tag = f"reverse-{router_id}"
    ...
    lines = [
        ...
        f'ROUTER_NAME="{sh_quote(row["name"])}"',
        ...
        # xray_bin_path() в агенте), а пустое значение означает "искать
        # самому". Раньше на owrt-remote была история, когда жёстко
...
def vps_terminal_commands(host):
    ...
    return [
        ...
        },
        {
            ...
            "note": "Слушают ли 80/443/8099/8444",
            "command": "ss -lntp | grep -E ':(80|443|8099|8444)'",
        },
        {
            "title": "Health Hub",
            "note": "Локальная health-проверка Hub",
            "command": "curl -sS http://127.0.0.1:8099/health",
        },
        {
...
<style>
:root{{color-scheme:dark;--bg:#0a0603;--panel:rgba(19,14,32,.88);--panel2:rgba(255,255,255,.07);--text:#f7f2ff;--muted:#b9adc9;--line:rgba(169,126,255,.25);--blue:#ff6a00;--green:#22c55e;--red:#fb7185;--amber:#f59e0b;--cyan:#ff9a3c;--teal:#2f8fff;--grid:rgba(255,106,0,.14)}}
*{{box-sizing:border-box}}body{{position:relative;min-height:100vh;margin:0;overflow-x:hidden;background-color:var(--bg);background-image:radial-gradient(circle at 12% 8%,rgba(255,106,0,.46),transparent 31%),radial-gradient(circle at 82% 12%,rgba(47,143,255,.38),transparent 30%),radial-gradient(circle at 50% 105%,rgba(47,143,255,.26),transparent 35%),linear-gradient(145deg,#0a0603,#12080a 48%,#070302),repeating-linear-gradient(0deg,transparent 0 30px,var(--grid) 31px),repeating-linear-gradient(90deg,transparent 0 30px,var(--grid) 31px);background-size:130% 130%,140% 140%,135% 135%,100% 100%,31px 31px,31px 31px;background-attachment:fixed;color:var(--text);font:14px/1.45 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;animation:bgFlow 28s ease-in-out infinite alternate}}
body::before{{content:"";position:fixed;inset:-25%;z-index:0;pointer-events:none;background:conic-gradient(from 0deg at 50% 50%,rgba(255,106,0,.05),rgba(47,143,255,.34),rgba(255,154,60,.22),rgba(245,158,11,.13),rgba(255,106,0,.05));filter:blur(54px);opacity:.7;animation:auraSpin 38s linear infinite}}
...
function render(list) {{
  ...
  cards.innerHTML = list.map(r => {{
    ...
    const model = (r.status && (r.status.model || r.status.board)) || 'Keenetic';
...
function nextEntryPort(list) {{
  ...
  // VodkinNET: явно другой диапазон, чем у owrt-remote (тот начинается с
  // 18080) — обе панели хранят порты в независимых, ничего не знающих
...
routerForm.addEventListener('submit', async (ev) => {{
  ...
  if (res.ok) {{
    ...
    await loadRouters();
    fillRouterForm(true);
    showRouterMsg('Роутер добавлен. Теперь открой Конфиг в его карточке и вставь его на роутер.');
  }} else {{
...
.mobileInput{{display:none;gap:8px;padding:8px;border-top:1px solid var(--line);background:rgba(255,255,255,.045)}}.mobileInput input{{flex:1;min-width:0;border:1px solid var(--line);border-radius:8px;padding:11px 12px;background:rgba(8,5,18,.76);color:var(--text);font:14px/1.2 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;outline:none}}.mobileInput input:focus{{border-color:rgba(255,154,60,.62);box-shadow:0 0 0 3px rgba(255,154,60,.12)}}.mobileInput button{{border:1px solid rgba(255,255,255,.12);border-radius:8px;padding:11px 12px;background:linear-gradient(135deg,#ff6a00,#2f8fff);color:#fff;font-weight:950;white-space:nowrap}}
#term{{flex:1 1 auto;min-width:0;min-height:0;margin:0;padding:12px;overflow:auto;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word;outline:none;cursor:text;background:rgba(0,0,0,.42);font:13px/1.34 "Cascadia Mono","Consolas","Liberation Mono",monospace;color:#f3d9c0;user-select:text;-webkit-user-select:text;touch-action:pan-y;scrollbar-width:thin;scrollbar-color:rgba(255,106,0,.72) rgba(255,255,255,.06)}}#term::selection,#term *::selection{{background:rgba(255,154,60,.34);color:#fff}}#term:focus{{box-shadow:inset 0 0 0 1px rgba(255,154,60,.30)}}#term::-webkit-scrollbar{{width:12px;height:12px}}#term::-webkit-scrollbar-track{{background:rgba(255,255,255,.06)}}#term::-webkit-scrollbar-thumb{{background:linear-gradient(180deg,#ff6a00,#ffb64d);border-radius:999px;border:3px solid rgba(10,6,18,.96)}}#term::-webkit-scrollbar-thumb:hover{{background:linear-gradient(180deg,#2f8fff,#ff9a3c)}}.term-error{{color:#fb7185;font-weight:900}}.term-warn{{color:#fde68a;font-weight:850}}.term-ok{{color:#bbf7d0;font-weight:850}}.term-info{{color:#ffb64d;font-weight:850}}.term-prompt{{color:#86efac;font-weight:900}}.term-metric{{color:#93c5fd;font-weight:850}}.term-muted{{color:#f0c9a0}}.term-inverse{{display:inline-block;background:#f3d9c0;color:#13091f;border-radius:3px;padding:0 3px;font-weight:900}}
.bad{{color:#fecdd3}}
...
async function explainTerminalError(source) {{
  ...
  try {{
    ...
    write(`\\r\\n[${{source}}]\\r\\nНе смог проверить SSH-туннель. Если страница открыта с мобильного интернета, проверь доступ к http://VPS_IP:8099/ и firewall VPS.\\r\\n`);
  }}
}}
function connect() {{
...
.mobileInput{display:none;gap:7px;padding:8px;border-top:1px solid var(--line);background:rgba(255,255,255,.045);flex:0 0 auto}.mobileInput textarea{flex:1;min-width:0;min-height:44px;max-height:96px;resize:vertical;border:1px solid var(--line);border-radius:8px;padding:11px 12px;background:rgba(8,5,18,.76);color:var(--text);font:14px/1.25 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;outline:none}.mobileInput textarea:focus{border-color:rgba(255,154,60,.62);box-shadow:0 0 0 3px rgba(255,154,60,.12)}.mobileInput button{border:1px solid rgba(255,255,255,.12);border-radius:8px;padding:11px 12px;background:linear-gradient(135deg,#ff6a00,#2f8fff);color:#fff;font-weight:950;white-space:nowrap}
...
*{{box-sizing:border-box}}
body{{position:relative;min-height:100vh;margin:0;overflow:hidden;background-color:var(--bg);background-image:radial-gradient(circle at 16% 12%,rgba(255,106,0,.48),transparent 30%),radial-gradient(circle at 84% 8%,rgba(255,154,60,.34),transparent 32%),radial-gradient(circle at 55% 105%,rgba(47,143,255,.24),transparent 36%),linear-gradient(145deg,#0a0603,#12080a 48%,#070302),repeating-linear-gradient(0deg,transparent 0 30px,var(--grid) 31px),repeating-linear-gradient(90deg,transparent 0 30px,var(--grid) 31px);background-size:130% 130%,140% 140%,135% 135%,100% 100%,31px 31px,31px 31px;background-attachment:fixed;color:var(--text);font:14px/1.45 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;display:grid;place-items:center;padding:18px;animation:bgFlow 28s ease-in-out infinite alternate}}
body::before{{content:"";position:fixed;inset:-28%;pointer-events:none;background:conic-gradient(from 0deg at 50% 50%,rgba(255,106,0,.06),rgba(47,143,255,.30),rgba(255,154,60,.22),rgba(255,154,60,.16),rgba(255,106,0,.06));filter:blur(58px);opacity:.74;animation:auraSpin 40s linear infinite}}
...
h1{{margin:0;font-size:18px;line-height:1.1;letter-spacing:0}}.appBanner{{position:relative;display:flex;align-items:center;justify-content:center;width:100%;min-height:54px;padding:10px 12px;border:1px solid rgba(255,154,60,.34);border-radius:8px;background:linear-gradient(110deg,rgba(255,154,60,.14),rgba(224,80,0,.24),rgba(47,143,255,.14));box-shadow:0 10px 24px rgba(224,80,0,.18),inset 0 1px 0 rgba(255,255,255,.10);font-size:17px;overflow:hidden}}.appBanner::before{{content:"";position:absolute;inset:0;background:linear-gradient(90deg,transparent,rgba(255,255,255,.18),transparent);transform:translateX(-120%);animation:bannerShine 6.2s ease-in-out infinite}}.appBanner span{{position:relative}}@keyframes bannerShine{{0%,45%{{transform:translateX(-120%)}}72%,100%{{transform:translateX(120%)}}}}
...
button{{width:100%;margin-top:13px;border:1px solid rgba(255,255,255,.12);border-radius:8px;padding:11px 12px;background:linear-gradient(135deg,#ff6a00,#2f8fff);color:#fff;font-weight:950;cursor:pointer;box-shadow:0 14px 30px rgba(224,80,0,.28)}}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def router_asset(self, path):
        ...
        if asset == "config":
            self.send_text(200, make_router_conf(row, hub_url))
            return
...
def cmd_print_router_config(args):
    with connect(args.db) as conn:
        init_db(conn)
...
def cmd_serve(args):
    ...
    auth = load_auth()
    # VodkinNET: the primary raw port (default 8099) carries the unauthenticated
...
def parser():
    ...
    ow = sub.add_parser("print-router-config", help="print router config text")
    ow.add_argument("--id", required=True)
    ow.add_argument("--hub-url", default="")
    ow.add_argument("--vps-host", default="127.0.0.1")
    ow.add_argument("--port", type=int, default=8099)
