...
Дальше:
  ...
  в конфиге всё равно будут проставлены (форма требует entry_port), но
  просто не используй этот канал. Мешает чисто косметически — можно
  занулить ADMIN_PORT в конфиге вручную и перезапустить агент, это ни
  на что не влияет.
