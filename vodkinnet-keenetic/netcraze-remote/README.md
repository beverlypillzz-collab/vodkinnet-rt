...
1. В панели Hub → "Добавить роутер": id, название, роль, VPS-хост,
   entry_port (сейчас обязательное поле формы — даже если реальной
   веб-морды NDMS на устройстве нет).
2. Скопируй блок конфига целиком (кнопка "Копировать") и вставь в
   `/opt/etc/netcraze-remote/netcraze-remote.conf` на Keenetic.
3. Если Entware слушает SSH не на дефолтном порту (см. список
   слушающих портов, который показал `install.sh`) — поправь `SSH_PORT`
   прямо в этом файле. В панели ничего трогать не нужно: агент сам
   репортит `admin_host`/`admin_port`/`ssh_host`/`ssh_port` в каждом
   heartbeat, Hub подхватывает актуальные значения автоматически.
